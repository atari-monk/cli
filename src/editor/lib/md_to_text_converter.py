...
def convert_md(md_input, text_output):
...
def copy_text(text_output, root):
...
def clear_fields(md_input, text_output):
...
def create_widgets(root):
    ...
    convert_button = tk.Button(button_frame, text="Convert to Text", command=lambda: convert_md(md_input, text_output))
    convert_button.grid(row=0, column=0, padx=5)

    copy_button = tk.Button(button_frame, text="Copy Text", command=lambda: copy_text(text_output, root))
    copy_button.grid(row=0, column=1, padx=5)

    clear_button = tk.Button(button_frame, text="Clear Fields", command=lambda: clear_fields(md_input, text_output))
...
def run_app():
    ...
    md_input, text_output = create_widgets(root)
